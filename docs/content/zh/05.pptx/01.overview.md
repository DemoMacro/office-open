...
每个异步方法都有对应的同步版本（如 `toBufferSync`）。完整列表见[导出](./19.export)页面。

| 方法                          | 返回值                       | 适用场景           |
| ----------------------------- | ---------------------------- | ------------------ |
| `Packer.toBuffer(pres)`       | `Buffer`                     | Node.js 文件 I/O   |
| `Packer.toBlob(pres)`         | `Blob`                       | 浏览器下载         |
| `Packer.toBase64String(pres)` | `string`                     | Data URL、API 载荷 |
| `Packer.toString(pres)`       | `string`                     | 调试、检查         |
| `Packer.toStream(pres)`       | `ReadableStream<Uint8Array>` | 流式传输大文件     |
