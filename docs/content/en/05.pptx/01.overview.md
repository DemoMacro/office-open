...
| `SlideMaster`, `SlideLayout`     | Slide masters and layouts for consistent design          |
| `NotesSlide`                     | Speaker notes for each slide                             |
...
| `OleFrame`                       | Embedded OLE objects                                     |
| `LockedCanvasFrame`              | Locked canvas for non-editable drawing objects           |

## Export Formats

Every async method has a synchronous counterpart (e.g., `toBufferSync`). See the [Export](./20.export) page for the full list.
...
See [Patching](./22.patch) for full documentation.
...
See [Parsing](./21.parsing) for full documentation.
