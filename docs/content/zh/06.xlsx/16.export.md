---
title: "导出"
description: "将工作簿导出为 Buffer、Blob、Base64、流等格式"
---

`Packer` 类将 `Workbook` 转换为各种输出格式。每个异步方法（`toBuffer`、`toBlob` 等）都有对应的同步版本（`toBufferSync`、`toBlobSync` 等）。`toStream` 方法仅有同步版本（无异步变体）。
