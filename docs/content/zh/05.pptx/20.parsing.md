---
title: "解析"
description: "使用 readPresentation 和 parsePptx 读取现有 .pptx 文件"
---

读取现有 `.pptx` 文件，将其转换为结构化数据，用于检查、修改或往返（round-trip）工作流。
...
- `readPresentation` 返回纯对象，兼容 JSON API — 使用 `{ shape: {...} }` 风格，而非 `new Shape()`。
