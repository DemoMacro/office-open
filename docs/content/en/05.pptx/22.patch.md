...
Append comments to existing slides, keyed by 0-based slide index (in `sldIdLst` order). Each comment reuses the `SlideCommentOptions` vocabulary from `generatePresentation` (see [Comments](./comments)). Authors are merged into `commentAuthors.xml` (deduped by name, ids continued) and per-slide comments are merged into `ppt/comments/commentN.xml`:
