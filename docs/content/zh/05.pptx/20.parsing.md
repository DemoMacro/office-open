...
高级 API，将 `.pptx` 文件解析为 `IPresentationOptions`，可直接传给 `new Presentation()`：
...
`parsePresentation` 返回 `IPresentationOptions` 对象：
...
- 解析后的数据可直接传给 `new Presentation(parsed)` 实现完整往返。
