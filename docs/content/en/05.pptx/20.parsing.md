...
The high-level API that parses a `.pptx` file into `IParsedPresentation`, which can be passed directly to `new Presentation()`:

```ts
import { parsePresentation, Presentation, Packer } from "@office-open/pptx";
import { readFileSync, writeFileSync } from "node:fs";

const parsed = parsePresentation(new Uint8Array(readFileSync("input.pptx")));

// Re-create and export
const pres = new Presentation({ slides: parsed.slides, masters: parsed.masters });
...
`parsePresentation` returns an `IParsedPresentation` object:
...
- Parsed data can be passed directly to `new Presentation({ slides: parsed.slides, masters: parsed.masters })`.
