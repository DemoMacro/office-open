...
`@office-open/docx` 和 `@office-open/pptx` 共享的 ZIP 打包工具。`createPacker` 工厂函数创建一个 `Packer` 对象，提供 `toBuffer`、`toBlob`、`toStream` 等方法。
...
压缩通过 Web Worker 实现非阻塞 DEFLATE。压缩策略与 Microsoft Office 一致：XML 条目使用 DEFLATE（级别 6），媒体文件（JPEG、PNG 等）使用 STORED（不压缩）。
