...
高级 API，将 `.pptx` 文件解析为 `IParsedPresentation`，可直接传给 `new Presentation()`：
...
`parsePresentation` 返回 `IParsedPresentation` 对象：

| 属性      | 类型                  | 说明                       |
| --------- | --------------------- | -------------------------- |
| `slides`  | `ISlideOptions[]`     | 幻灯片选项数组             |
| `masters` | `IMasterDefinition[]` | 母版定义（仅多母版时存在） |
...
- 解析后的数据可直接传给 `new Presentation({ slides: parsed.slides, masters: parsed.masters })`。
