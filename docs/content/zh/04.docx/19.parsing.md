...
| `settings.view`、`settings.zoom`、`settings.defaultTabStop` | `word/settings.xml`   | 文档设置                           |
| `settings.evenAndOddHeaders`                                | `word/settings.xml`   | 页眉页脚模式                       |
| `settings.trackRevisions`                                   | `word/settings.xml`   | 修订追踪                           |
| `settings.updateFields`                                     | `word/settings.xml`   | 自动更新域                         |
| `background`                                                | `word/settings.xml`   | 文档背景                           |
| `settings.docVars`                                          | `word/settings.xml`   | 文档变量                           |
...
| JSON 键             | 类型     | 描述                                                 |
| ------------------- | -------- | ---------------------------------------------------- |
| `commentRangeStart` | `object` | 批注范围起始标记（`id`、`displacedByCustomXml?`）    |
| `commentRangeEnd`   | `object` | 批注范围结束标记（`id`、`displacedByCustomXml?`）    |
...
| `bookmarkEnd`       | `object` | 书签结束标记（`id`、`displacedByCustomXml?`）        |
| `picture`           | `object` | 内嵌图片，包含 `type`、`data`、`transformation`      |
