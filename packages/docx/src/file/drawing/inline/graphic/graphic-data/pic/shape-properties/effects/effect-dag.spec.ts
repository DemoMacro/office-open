...
describe("createEffectDag", () => {
  ...
  it("should create an effectDag with glow", () => {
    ...
    expect(tree).to.deep.equal({
      "a:effectDag": [
        {
          "a:glow": [{ _attr: { rad: 50800 } }, { "a:srgbClr": { _attr: { val: "FF0000" } } }],
        },
      ],
    });
  });
  ...
  it("should create an effectDag with alphaBiLevel", () => {
    const tree = new Formatter().format(createEffectDag({ alphaBiLevel: { threshold: 50000 } }));
    expect(tree).to.deep.equal({
  ...
  it("should create an effectDag with alphaCeiling and alphaFloor", () => {
    const tree = new Formatter().format(createEffectDag({ alphaCeiling: true, alphaFloor: true }));
    expect(tree).to.deep.equal({
  ...
  it("should create an effectDag with alphaModulateFixed", () => {
    const tree = new Formatter().format(createEffectDag({ alphaModulateFixed: { amount: 75000 } }));
    expect(tree).to.deep.equal({
  ...
  it("should create an effectDag with tint", () => {
    const tree = new Formatter().format(createEffectDag({ tint: { hue: 2700000, amount: 30000 } }));
    expect(tree).to.deep.equal({
