...
`@office-open/docx` 和 `@office-open/pptx` 共享的单一图表描述符（`chartSpaceDesc`，通过 `type` 字段区分柱形图、折线图、饼图等所有图表类型）和 SmartArt 组件。图表 XML 生成和解析均通过描述符驱动。
...
覆盖 vml、office、word、excel、presentation 五个命名空间的完整结构化 VML 词汇——形状、描边、文本框，以及文本框、OLE 对象和单元格批注使用的旧版锚定。
...
异步方法使用 Web Worker 实现非阻塞 DEFLATE。同步方法会阻塞主线程，但避免了 Worker 开销。压缩策略与 Microsoft Office 一致：XML 条目使用 DEFLATE（级别 1，SuperFast）；媒体按类型区分——已压缩格式（JPEG、PNG、GIF）使用 STORE，其余（EMF/WMF/BMP/TIFF）使用 DEFLATE 级别 6 / Normal。
