...
export class Area3DChart extends XmlComponent {
  public constructor(options: Area3DChartOptions) {
    super("c:area3DChart");
    this.root.push(wrapEl("c:grouping", chartAttr({ val: "standard" })));

    for (let i = 0; i < options.series.length; i++) {
      this.root.push(new Area3DSeries(i, options.series[i], options.categories));
    }
...
class Area3DSeries extends XmlComponent {
