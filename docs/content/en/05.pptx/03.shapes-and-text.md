...
`outline` and `shadow` accept `true` for a sensible default or a full options object — see [Outline](./15.effects-and-fills#outline) and [Effects](./15.effects-and-fills#effects) for the complete shapes. See [Links](./09.links) for `hyperlink` (external `url` or internal `slide` jump).
