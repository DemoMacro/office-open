...
获取度量属性为数字，或原样保留 UniversalMeasure/百分比 token（用于 `ST_TwipsMeasure`、`ST_MeasurementOrPercent`、`CT_TblWidth/@w`）。百分比的 fiftieths 需传 `"pct"` 作为 `type` 以保持字符串：

```ts
import { attrMeasure } from "@office-open/xml";

attrMeasure(spacing, "w:before"); // 240（数字）— 纯数字 token
attrMeasure(width, "w:w", "pct"); // "5000" 原样保留（百分比的 fiftieths）
```
...
| `attrMeasure(element, name, type?)` | `number \| string \| undefined` | 度量属性（保留单位）       |
