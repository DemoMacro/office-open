...
import type { ImageOptions, ChartAnchorOptions } from "@file/drawing";
import type { WorkbookOptions } from "@file/file";
import { aggregate, collectUniqueValues } from "@file/pivot";
import type { PivotSourceData, PivotTableOptions } from "@file/pivot";
import { DefaultTheme } from "@file/theme";
import type { TablePartReference } from "@file/workbook";
import { buildWorksheetXml, type WorksheetContext } from "@file/worksheet";
import {
  APP_PROPS_XML,
  ChartSpace,
  Relationships,
...
import { calcChainDesc } from "./descriptors/calc-chain";
import { chartsheetDesc } from "./descriptors/chartsheet";
import { commentsDesc, vmlNotesDesc } from "./descriptors/comments";
import { drawingDesc } from "./descriptors/drawing";
import { externalLinkDesc } from "./descriptors/external-link";
import { pivotCacheDefDesc, pivotCacheRecordsDesc } from "./descriptors/pivot-cache";
import { pivotTableDesc } from "./descriptors/pivot-table";
import { sharedStringsDesc } from "./descriptors/shared-strings";
import { stylesDesc } from "./descriptors/styles";
import { tableDesc } from "./descriptors/table";
import {
  workbookDesc,
  buildTablePartsXml,
  buildExternalReferencesXml,
} from "./descriptors/workbook";
...
export function compileWorkbook(
  ...
  const sheets: import("@file/workbook").SheetDefinition[] = [];
  let sheetId = 1;
  let rId = 1;
  ...
  // Theme
  mapping["Theme"] = {
    data: XML_DECL + new DefaultTheme().serialize(),
  ...
  for (let i = 0; i < ctx.charts.array.length; i++) {
    ...
    mapping[`Chart${i}`] = {
      data: XML_DECL + chartData.chartSpace.serialize(),
...
function extractPivotSourceData(
  rows: import("@file/worksheet").RowOptions[],
  sourceRef: string,
): PivotSourceData {
...
function renderPivotSheetData(
  ...
  worksheetConfigs: import("@file/worksheet").WorksheetOptions[],
