...
- **`parseDocument`** — High-level round-trip API that returns `IPropertiesOptions`
- **`parseDocx`** — Low-level access to the raw XML parts of a `.docx` file

## Round-Trip (parseDocument)

The `parseDocument` function parses a `.docx` file into the same `IPropertiesOptions` format used by the `Document` constructor, enabling full round-trip (export → parse → re-export):
