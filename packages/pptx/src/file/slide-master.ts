...
function buildBackgroundXml(bg?: BackgroundOptions): string {
  ...
  const fillXml = buildFill(bg.fill ?? { type: "none" }).serialize();
  let effectsXml = "";
  if (bg.effects) {
