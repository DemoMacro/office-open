...
- **`parseDocument`** — 高层级往返 API，返回 `ISectionOptions[]`

## 往返（parseDocument）

`parseDocument` 函数将 `.docx` 文件解析为 `Document` 构造函数使用的 `ISectionOptions[]` 格式，支持完整的往返流程（导出 → 解析 → 重新导出）：
