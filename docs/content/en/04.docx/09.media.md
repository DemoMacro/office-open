...
Word documents can embed various media types. This package provides support for images through the `picture` run type, which is the primary media type used in DOCX files.
...
| ------------- | ---------------------------- | ------------- |
| Raster images | JPG, PNG, GIF, BMP, TIF, ICO | `{ picture }` |
| Vector images | SVG (with fallback)          | `{ picture }` |
| EMF/WMF       | EMF, WMF                     | `{ picture }` |
