...
```json
{
  "paragraph": {
    "children": [
      ...
      { "ruby": { "base": "汉字", "text": "hànzì", "languageId": "zh-CN" } }
    ]
  }
}
```
...
| `text`         | `string`                                                                                      | 拼音标注文本                 |
| `alignment`    | `"center" \| "distributeLetter" \| "distributeSpace" \| "left" \| "right" \| "rightVertical"` | 对齐方式（默认：`"center"`） |
| `fontSize`     | `number`                                                                                      | 拼音文本字号（磅）           |
| `raise`        | `number`                                                                                      | 垂直偏移量（磅）             |
| `baseFontSize` | `number`                                                                                      | 基础文本字号（磅）           |
| `languageId`   | `string`                                                                                      | 语言标识符（如 `"ja-JP"`）   |
| `dirty`        | `boolean`                                                                                     | 标注是否为脏（需重算）       |
