...
`parse` 函数将 XML 字符串转换为 `Element` 树。
...
| `captureSpacesBetweenElements` | `boolean` | `false` | 保留元素间的纯空白文本节点    |
...
每个解析后的节点是一个 `Element` 对象——完整接口见[类型参考](./types)。元素通常使用 `name`、`attributes` 和 `elements`：
