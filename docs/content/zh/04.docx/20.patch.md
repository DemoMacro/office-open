...
直接编辑节列表 — 替换整节内容或追加新节。每条替换项用 0-based `index` 指明目标节（0 = 从文档开头算起）：
    ...
    replace: [
      {
        index: 0,
        section: {
          children: [{ paragraph: { heading: "Heading1", children: ["第一章 — 已替换"] } }],
        },
      },
    ],
    append: [
      {
        children: [{ paragraph: { children: ["附录"] } }],
...
- 未给 `properties` 时，被替换的节保留其现有页面设置；给了 `properties` 则由新设置接管。
- 追加的节插入在最后 body 级 `sectPr` 之前，每节以自己的分节符收尾。未给 `properties` 的节继承最后一节的页面设置。
- 子内容与 `append` 走同一序列化管线（段落、表格、图片、超链接）。
- 修补进来的节不支持 `headers`/`footers` — 它们需要新建 part；请改用 `generateDocument`。
...
const result = await patchDocument({
  ...
  comments: {
    paragraphs: [
      {
        index: 0,
        comments: [{ author: "Alice", children: ["审阅开篇段落。"] }],
      },
    ],
    placeholders: {
...
| `sections`              | `{ replace?, append? }`          | —                            | 节编辑：`replace` 是 `{ index, section }` 数组 |
...
- 每个占位符的所有出现都会被替换，而非仅第一处 — 正文、页眉、页脚一视同仁。
