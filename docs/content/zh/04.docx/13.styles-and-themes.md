...
通过 `styles.external` 选项（原始 XML 字符串）从现有文档导入整个 `styles.xml`。未定义的内置样式仍由默认工厂生成；此处定义的样式按 `styleId` 覆盖默认内置样式。
...
// styles.external 是 <w:styles> 部件的原始 XML —— 例如从现有文档的 word/styles.xml 读取
await generateDocument({
  styles: {
