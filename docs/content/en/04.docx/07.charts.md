...
Scatter charts plot each category against its value (`categories` for the X axis, `values` for the Y axis):
...
```ts [TypeScript]
{
  chart: {
    type: "scatter",
    series: [{ name: "Dataset 1", xValues: [1, 2, 3, 4, 5], yValues: [2, 4, 1, 5, 3] }],
    transformation: { width: "13.2cm", height: "7.9cm" },
  },
}
```
