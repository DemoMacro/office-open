...
export class SurfaceChart extends XmlComponent {
  public constructor(options: SurfaceChartOptions) {
    super("c:surfaceChart");
    if (options.wireframe) {
      this.root.push(wrapEl("c:wireframe", chartAttr({ val: true })));
    }

    for (let i = 0; i < options.series.length; i++) {
      this.root.push(new SurfaceSeries(i, options.series[i], options.categories));
    }
...
class SurfaceSeries extends XmlComponent {
