...
Edit the section list itself — replace a whole section's content or append new sections. Sections are keyed by 0-based index (0 = from the document start):
...
| `sections`              | `{ replace?, append? }`          | —                            | Section collection edits keyed by 0-based index                                   |
