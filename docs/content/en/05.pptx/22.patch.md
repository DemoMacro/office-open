      ...
      children: [
        {
          shape: {
            ...
            height: "2.1cm",
            textBody: {
              paragraphs: [{ children: [{ text: "Hello {{name}}!" }] }],
            },
          },
        },
      ],
...
Placeholder keys are wrapped in delimiters (default `{{` and `}}`) and matched against the slide text. Each value is inline run-level content, reusing the same `TextRunOptions` vocabulary as `generatePresentation`: a single run, an array of runs, or a plain string shorthand.
...
Append new slides, replace existing ones by index, or remove slides — all keyed by 0-based index (in `sldIdLst` order). Each entry reuses the full `SlideOptions` vocabulary from `generatePresentation`:
...
const result = await patchPresentation({
  ...
  slides: {
    ...
    // Drop the appendix slides from a 10-page template — removal unwires
    // the sldIdLst entry, the presentation relationship, the slide part
    // with its rels, and the content-type Override.
    remove: [1, 2],
  },
});
```

Appended slides are serialized through the same slide stringifier as generation and wired into `sldIdLst`, the presentation relationships, and `[Content_Types].xml`. Each appended slide inherits the template's first slide layout — it cannot reference layouts, masters, or media that the template does not already contain. Replaced slides keep their existing identity (`sldId`, `rId`, and relationships). Removed slides take their notes slides with them; shared parts (layouts, media, charts) stay for the remaining slides. Indices in `remove` refer to the deck as it entered the patch — the same original order that `replace` is keyed by; removals are applied after appends.

## Comments

Append comments to existing slides, keyed by 0-based slide index (in `sldIdLst` order). Each comment reuses the `SlideCommentOptions` vocabulary from `generatePresentation` (see [Comments](./18.comments.md)). Authors are merged into `commentAuthors.xml` (deduped by name, ids continued) and per-slide comments are merged into `ppt/comments/commentN.xml`:
...
| `placeholders`          | `Record<string, TextRunOptions \| TextRunOptions[] \| string>` | —                            | Delimiter-wrapped placeholder name → run content                  |
| `findReplace`           | `Record<string, TextRunOptions \| TextRunOptions[] \| string>` | —                            | Literal find string → run content (no delimiters)                 |
| `coreProperties`        | `Partial<CorePropertiesOptions>`                               | —                            | Core metadata override, merged over existing values               |
| `slides`                | `{ append?, replace?, remove? }`                               | —                            | Append slides, replace by 0-based index, or remove by index       |
...
- Every occurrence of a placeholder is replaced, not just the first — across all slides, masters, layouts, and notes.
