...
export interface SlideMasterOptions {
  ...
  /**
   * Verbatim p:txStyles XML (title/body/other text styles — CT_SlideMasterTextStyles).
...
/**
 * Default p:txStyles for a fresh master — MS Office's standard title/body/other
 * text styles (9 outline levels each). Used when no parsed textStyles is present;
 * a round-tripped master supplies its own so custom typography survives.
 */
const DEFAULT_TX_STYLES =
