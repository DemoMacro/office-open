...
```ts
import { Paragraph } from "@office-open/docx";
...
Note: The `lineRule` property has different values depending on the version of Word you are using. The `EXACTLY` value is only available in Word 2016 and above. Use `EXACT` for greater support, including LibreOffice etc. Read this issue for more information: https://github.com/DemoMacro/office-open/issues/1773.
...
```ts
import { HeadingLevel, Paragraph } from "@office-open/docx";
...
Example: https://github.com/DemoMacro/office-open/blob/main/packages/docx/demo/15-page-break-before.ts
