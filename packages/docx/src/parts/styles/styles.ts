...
export function parseStyleDefinitions(
  ...
  for (const child of el.elements ?? []) {
    if (child.name === "w:docDefaults") {
      ...
      // Table styles are fully structured (no verbatim _raw round-trip) —
      // parsed into TableStyleOptions, generate re-emits via stringifyTableStyle.
      ...
      const isBuiltin = Boolean(defaultField) || BUILTIN_STYLE_IDS.has(styleOpts.id);
      // Builtins round-trip verbatim: the factory's fixed defaults cannot
      // reproduce a source document's customized builtin (e.g. a recolored
      // Heading1). Overridable builtins are also mapped into default.<field>
      // so users can read and edit them; generate re-emits the edited version
      // via the factory (default.<field> wins over the verbatim entry).
      if (isBuiltin) {
      ...
      // Custom paragraph/character styles — structured only (no verbatim
      // _raw). The round-trip path re-emits them via stringifyParagraphStyle/
      // stringifyCharacterStyle, so every field must round-trip losslessly.
