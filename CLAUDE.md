...
- **Parts**: one module per OOXML XML part. docx/xlsx co-locate types and the `<part>Desc` descriptor (`parts/<part>/` for complex, `parts/<part>.ts` for simple); pptx keeps descriptors in a `parts/descriptors/` layer with public types in `shared/<domain>/`. Both patterns are fine if the package is internally consistent. Cross-part shared types live in `shared/`.
