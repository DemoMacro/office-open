...
import type { ParagraphOptions } from "@file/shape/paragraph/paragraph";
...
function toReflectionCore(opts: import("@file/drawingml/effects").ReflectionOptions) {
...
export const pictureDesc: CustomDescriptor<PictureDescriptorOptions> = {
  ...
  stringify(opts, ctx) {
    ...
    pptx.addImage(fileName, {
      ...
      data: opts.data,
      transformation: {
        pixels: { x: opts.width ?? 0, y: opts.height ?? 0 },
        emus: {
          x: convertPixelsToEmu(opts.width ?? 0),
          y: convertPixelsToEmu(opts.height ?? 0),
        },
      },
    });
...
function stringifySpPr(opts: ShapeDescriptorOptions, ctx: WriteContext): string {
  ...
  if (hasTransform) {
    const transformOpts: Record<string, number | boolean> = {};
    if (opts.x !== undefined) transformOpts.x = convertPixelsToEmu(opts.x);
    if (opts.y !== undefined) transformOpts.y = convertPixelsToEmu(opts.y);
    if (opts.width !== undefined) transformOpts.width = convertPixelsToEmu(opts.width);
    if (opts.height !== undefined) transformOpts.height = convertPixelsToEmu(opts.height);
    if (opts.flipHorizontal !== undefined) transformOpts.flipHorizontal = opts.flipHorizontal;
    if (opts.rotation !== undefined) transformOpts.rotation = opts.rotation;
    const xml = stringify(transform2DDesc, transformOpts as any, ctx);
...
export function readSpPr(spPr: XmlElement, ctx: ReadContext): Partial<ShapeDescriptorOptions> {
  ...
  if (xfrm) {
    ...
    if (transformOpts.x !== undefined) result.x = Math.round(transformOpts.x / 9525);
    if (transformOpts.y !== undefined) result.y = Math.round(transformOpts.y / 9525);
    if (transformOpts.width !== undefined) result.width = Math.round(transformOpts.width / 9525);
    if (transformOpts.height !== undefined) result.height = Math.round(transformOpts.height / 9525);
