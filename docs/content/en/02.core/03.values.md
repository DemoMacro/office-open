...
Accepts values with unit suffixes: `mm`, `cm`, `in`, `pt`, `pc`, `pi`. Geometry/sizing APIs additionally accept `px` (96 DPI pixels) as a project extension — `px` is not part of OOXML's `ST_UniversalMeasure`, and is converted to EMU (or TWIP) on input rather than written to XML verbatim.
