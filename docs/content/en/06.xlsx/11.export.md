...
The `Packer` class converts a `Workbook` into various output formats. Every async method (`toBuffer`, `toBlob`, etc.) has a synchronous counterpart (`toBufferSync`, `toBlobSync`, etc.). The `toStream` method is synchronous-only (no async variant).
...
await Packer.toBuffer(wb, { compression: { xml: 9 } });
