...
Add a comment with the `comment` child, placed inline next to the text it covers. The library allocates the comment id, emits the range markers and reference, and registers the comment entry for you — no manual id:
...
Track moved content with `moveFromRangeStart`/`moveFromRangeEnd` (source) and `moveToRangeStart`/`moveToRangeEnd` (destination) range markers. Wrap the moved text between each start/end pair:
      ...
      children: [
        {
          paragraph: {
            children: [
              { moveFromRangeStart: { id: 0, author: "Author", date: "2024-01-15T10:30:00Z" } },
              "moved text",
        ...
        },
        {
          paragraph: {
            children: [
              { moveToRangeStart: { id: 1, author: "Author", date: "2024-01-15T10:30:00Z" } },
              "moved text",
              { moveToRangeEnd: { id: 1 } },
            ],
          },
...
Range starts accept `id` (required) plus `author`, `date`, `name`, and table-column scope (`colFirst`/`colLast`); range ends take just `{ id }`.
