...
describe("PermStart", () => {
  ...
  it("should create a permStart with ed (individual user)", () => {
    const tree = new Formatter().format(new PermStart({ id: "range2", ed: "john@example.com" }));
    expect(tree).to.deep.equal({
      "w:permStart": {
        _attr: {
          "w:id": "range2",
  ...
  it("should create a permStart with column range", () => {
    const tree = new Formatter().format(new PermStart({ id: "range3", colFirst: 1, colLast: 3 }));
    expect(tree).to.deep.equal({
      "w:permStart": {
        _attr: {
          "w:id": "range3",
