...
const buffer = generateDocumentSync({/* options */});
```
...
const stream = generateDocumentStream({/* options */});
Readable.fromWeb(stream).pipe(createWriteStream("output.docx"));
```

## 压缩选项

所有函数均支持压缩控制。默认行为与 Microsoft Office 一致：XML 使用 DEFLATE 级别 1（SuperFast）；媒体按类型区分——已压缩格式（PNG/JPEG/GIF）使用 STORE，其余（EMF/WMF/BMP/TIFF）使用 DEFLATE 级别 6 / Normal。
