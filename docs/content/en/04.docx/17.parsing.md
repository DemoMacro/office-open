...
- **`parseDocument`** — High-level round-trip API that returns `PropertiesOptions`
...
The `parseDocument` function parses a `.docx` file into the same `PropertiesOptions` format used by the `Document` constructor, enabling full round-trip (export → parse → re-export):
...
| `styles`                             | `word/styles.xml`     | Style definitions (paragraph, character, default) |
