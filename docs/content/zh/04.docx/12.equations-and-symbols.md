...
```json
{ "fraction": { "numerator": ["a + b"], "denominator": ["b"], "fractionType": "bar" } }
```
...
| 属性         | 类型                    | 说明                 |
| ------------ | ----------------------- | -------------------- |
| `properties` | `RubyPropertiesOptions` | 必需的拼音布局属性   |
| `base`       | `RubyContentOptions`    | 格式化的基础文本 run |
| `text`       | `RubyContentOptions`    | 格式化的标注文本 run |

`RubyPropertiesOptions` 需要 `alignment`、`fontSize`、`raise`、`baseFontSize` 和 `languageId`；`dirty` 可选。
