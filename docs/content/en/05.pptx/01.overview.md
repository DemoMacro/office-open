...
Read existing `.pptx` files into `ISlideOptions[]` for inspection or round-trip workflows:

```ts
import { readPresentation } from "@office-open/pptx";

const slides = readPresentation(buffer);
// slides[0].children — shapes, tables, charts, etc.
// slides[0].background — slide background
// slides[0].transition — slide transition
// slides[0].notes — speaker notes
```
