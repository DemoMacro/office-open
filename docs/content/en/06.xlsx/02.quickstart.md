...
`generateWorkbook` supports multiple output formats via the optional second argument. The synchronous variant is `generateWorkbookSync`; the streaming variant is `generateWorkbookStream`. See the [Export](./export) page for the full list.
...
| `generateWorkbookStream(opts)`               | `ReadableStream<Uint8Array>` | Streaming output        |
