...
const myElementDesc = element<MyOptions>("w:myElement")
  .attr("val", { ns: "w" })
  .child("items", { tag: "w:item", desc: itemDesc })
  .build();
```

Core provides `CustomDescriptor<T>`, `ElementDescriptor<T>`, and the `DescriptorBuilder` fluent API. Use `stringify(desc, value, ctx)` and `parse(desc, element, ctx)` at runtime.
