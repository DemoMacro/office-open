---
title: 解析器
description: 使用 ParsedArchive 解析和修改现有的 OOXML 文档
---

解析器将 `.docx`、`.pptx` 和 `.xlsx` 文件读取为 `ParsedArchive`，提供键值存储接口来访问和修改 XML 部件。
...
将 `ParsedArchive` 与 `@office-open/xml` 查询工具结合使用，可以检查和修改 XML 内容：
