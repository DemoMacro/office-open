---
title: "@office-open/docx"
description: 使用声明式 TypeScript API 生成、解析和修补 .docx 文档
---

## 安装

::code-group

```bash [pnpm]
pnpm add @office-open/docx
```

```bash [npm]
npm install @office-open/docx
```

```bash [yarn]
yarn add @office-open/docx
```

```bash [bun]
bun add @office-open/docx
```

::

## 快速开始

::api-example{type="docx"}

```json [JSON]
{
  "sections": [
    {
      "children": [{ "paragraph": { "children": ["Hello World"] } }]
    }
  ]
}
```

```ts [Class]
import { Document, Packer, Paragraph, TextRun } from "@office-open/docx";

const doc = new Document({
  sections: [
    {
      children: [
        new Paragraph({
          children: [new TextRun("Hello World")],
        }),
      ],
    },
  ],
});

const buffer = await Packer.toBuffer(doc);
```

::

## 主要组件

| 组件                                                     | 说明                                 |
| -------------------------------------------------------- | ------------------------------------ |
| `Document`                                               | 文档根容器，包含节和属性             |
| `Paragraph`                                              | 文本段落，支持格式化、对齐和间距     |
| `TextRun`                                                | 内联文本，支持字体、大小、颜色和样式 |
| `Table`, `TableRow`, `TableCell`                         | 结构化表格，支持合并单元格和边框     |
| `ImageRun`                                               | 嵌入图片，支持变换和定位             |
| `ChartRun`                                               | 柱状图、折线图、饼图、面积图和散点图 |
| `SmartArtRun`                                            | 预定义 SmartArt 图示                 |
| `Header`, `Footer`                                       | 节页眉和页脚                         |
| `FootnoteReferenceRun`, `EndnoteReferenceRun`            | 脚注和尾注                           |
| `TableOfContents`                                        | 自动生成的目录                       |
| `Bookmark`                                               | 内部书签和交叉引用                   |
| `Math`, `MathRun`                                        | 数学公式（Office MathML / OMML）     |
| `Styles`                                                 | 声明式样式定义                       |
| `ExternalHyperlink`, `InternalHyperlink`                 | 外部和内部超链接                     |
| `Comment`, `CommentReference`                            | 文档批注，支持作者和日期跟踪         |
| `TextBox`                                                | 浮动文本框                           |
| `CheckBox`                                               | 表单复选框                           |
| `StructuredDocumentTagRun`, `StructuredDocumentTagBlock` | 结构化文档标签（SDT）                |
| `InsertedTextRun`, `DeletedTextRun`                      | 修订跟踪 — 插入和删除                |
| `Numbering`, `LevelFormat`                               | 编号和项目符号列表，支持多级         |
| `SymbolRun`, `PageBreak`, `ColumnBreak`                  | 特殊字符、分页符和分栏符             |
| `SectionProperties`                                      | 页面大小、边距、方向、分栏和页面边框 |

## 补丁

通过替换 `{{占位符}}` 标记修改现有 `.docx` 模板：

```ts
import { patchDocument, PatchType, TextRun } from "@office-open/docx";

const result = await patchDocument({
  outputType: "nodebuffer",
  data: templateBuffer,
  patches: {
    name: { type: PatchType.PARAGRAPH, children: [new TextRun("John Doe")] },
  },
});
```

详见[补丁](./18.patch)页面。

## 导出格式

每个异步方法都有对应的同步版本（如 `toBufferSync`）。完整列表见[导出](./16.export)页面。

| 方法                         | 返回值                       | 适用场景           |
| ---------------------------- | ---------------------------- | ------------------ |
| `Packer.toBuffer(doc)`       | `Buffer`                     | Node.js 文件 I/O   |
| `Packer.toBlob(doc)`         | `Blob`                       | 浏览器下载         |
| `Packer.toBase64String(doc)` | `string`                     | Data URL、API 载荷 |
| `Packer.toString(doc)`       | `string`                     | 调试、检查         |
| `Packer.toStream(doc)`       | `ReadableStream<Uint8Array>` | 流式传输大文件     |
