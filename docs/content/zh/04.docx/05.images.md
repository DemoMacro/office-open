...
使用 `{ picture: {...} }` 在段落中嵌入图片。支持 JPG、PNG、GIF、BMP、SVG 等格式。
...
```ts [TypeScript]
{
  picture: {
    ...
    transformation: { width: "5.3cm", height: "5.3cm" },
    sourceRectangle: {
      left: 1000, // 裁剪左边（ST_Percentage；1000 = 1%）
      top: 1000, // 裁剪上边（ST_Percentage；1000 = 1%）
      right: 1000, // 裁剪右边（ST_Percentage；1000 = 1%）
      bottom: 1000, // 裁剪下边（ST_Percentage；1000 = 1%）
    },
  },
}
```
