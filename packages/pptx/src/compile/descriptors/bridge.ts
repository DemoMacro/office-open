...
export function isPlainObject(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !("toXml" in value);
}

/**
 * Serialize a legacy SlideChild to XML using descriptors when possible,
 * falling back to the class-based coerce+toXml() for unsupported types.
 */
export function stringifyChild(child: LegacySlideChild, ctx: PptxWriteContext): string | undefined {
  // Class instance — use serialize()
  if (!isPlainObject(child)) {
    return (child as { serialize(): string }).serialize();
  }

  // JSON object — try descriptor first
