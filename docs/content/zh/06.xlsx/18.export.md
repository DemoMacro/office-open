...
// 异步（非阻塞）
const buffer = await generateWorkbook({/* 选项 */});
...
const buffer = generateWorkbookSync({/* 选项 */});
```
...
const stream = generateWorkbookStream({/* 选项 */});
Readable.fromWeb(stream).pipe(createWriteStream("output.xlsx"));
```

## 压缩选项

所有函数均接受压缩控制参数。默认行为与 Microsoft Office 一致：XML 使用 DEFLATE 级别 1（SuperFast）；媒体按类型区分——已压缩格式（PNG/JPEG/GIF）使用 STORE，其余（EMF/WMF/BMP/TIFF）使用 DEFLATE 级别 6 / Normal。
...
| `compression.media` | `number` | 6      | 仅可压缩媒体；PNG/JPEG/GIF 始终 STORE |
