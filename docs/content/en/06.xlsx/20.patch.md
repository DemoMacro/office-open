...
Append cell comments (notes) to existing worksheets, keyed by sheet name. Each comment reuses the `CommentOptions` vocabulary from `generateWorkbook` (see [Comments](./comments)) — `cell`, `author`, and `text`:
