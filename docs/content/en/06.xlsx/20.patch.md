...
Append new worksheets, replace existing ones by sheet name, or remove worksheets. Each entry reuses the full `WorksheetOptions` vocabulary from `generateWorkbook`:
...
const result = await patchWorkbook({
  ...
  worksheets: {
    ...
    // Drop the draft sheets from a template — removal unwires the
    // workbook.xml entry, the workbook relationship, the sheet part with
    // its rels, and the content-type Override.
    remove: ["Report Draft"],
  },
});
```
...
Removal also renumbers what points at the sheet list: `definedName` entries scoped to a removed sheet are dropped, survivors' `localSheetId` (and the workbook view's `activeTab`) shift to the post-removal indices.

## Comments

Append cell comments (notes) to existing worksheets, keyed by sheet name. Each comment reuses the `CommentOptions` vocabulary from `generateWorkbook` (see [Comments](./14.comments)) — `cell`, `author`, and `text`:
...
| `worksheets`            | `{ append?, replace?, remove? }`   | —                            | Append worksheets, replace by name, or remove by name                         |
