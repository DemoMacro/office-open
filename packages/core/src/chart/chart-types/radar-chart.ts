...
export class RadarChart extends XmlComponent {
  public constructor(options: RadarChartOptions) {
    super("c:radarChart");

    this.root.push(wrapEl("c:radarStyle", chartAttr({ val: options.radarStyle ?? "standard" })));

    for (let i = 0; i < options.series.length; i++) {
      this.root.push(new RadarSeries(i, options.series[i], options.categories));
    }
...
class RadarSeries extends XmlComponent {
