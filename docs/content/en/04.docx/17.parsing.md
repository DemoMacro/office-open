...
- **`parseDocument`** — High-level round-trip API that returns `ISectionOptions[]`

## Round-Trip (parseDocument)

The `parseDocument` function parses a `.docx` file into the same `ISectionOptions[]` format used by the `Document` constructor, enabling full round-trip (export → parse → re-export):
...
const sections = parseDocument(data);
...
The returned sections contain the same structure as the input format:
