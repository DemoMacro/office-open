...
1. 使用声明式 JSON 对象描述文档结构，传入 `generateDocument()`、`generatePresentation()` 或 `generateWorkbook()` 等函数
2. 每个 JSON 键映射到对应的 OOXML 元素，生成合法的 XML 标记
3. 函数内部将所有内容序列化为 Office Open XML 包（ZIP 归档）
4. 你将获得一个可直接保存或提供的 `Buffer`、`Blob`、`Base64` 字符串或流
