...
Use `Chart` to embed charts in slides. Charts are configured with a flat options object — pass `type`, `categories`, and `series` directly.
...
```ts [Class]
import { Chart } from "@office-open/pptx";
...
| `"bubble"`   | Bubble chart (requires xValues/yValues/sizes) |
