      ...
      children: [
        {
          shape: {
            ...
            height: "2.1cm",
            textBody: {
              paragraphs: [{ children: [{ text: "你好 {{name}}！" }] }],
            },
          },
        },
      ],
...
占位符名称用分隔符包裹（默认 `{{` 和 `}}`），与幻灯片文本匹配。每个值是内联运行级内容，复用 `generatePresentation` 相同的 `TextRunOptions` 词汇表：单个 run、run 数组，或纯字符串简写。
...
追加新幻灯片、按索引（0-based，`sldIdLst` 顺序）替换现有幻灯片、或删除幻灯片。每项复用 `generatePresentation` 完整的 `SlideOptions` 词汇：
...
const result = await patchPresentation({
  ...
  slides: {
    ...
    // 从 10 页模板中删掉附录页 — 删除会解除 sldIdLst 条目、
    // 演示文稿关系、幻灯片 part 及其 rels、content-type Override 的全部接线。
    remove: [1, 2],
  },
});
```

追加的幻灯片经与生成相同的幻灯片序列化器处理，并接入 `sldIdLst`、演示文稿关系和 `[Content_Types].xml`。每张追加的幻灯片继承模板的第一个幻灯片布局 — 它无法引用模板中不存在的布局、母版或媒体。被替换的幻灯片保留其原有身份（`sldId`、`rId` 和关系）。被删除的幻灯片会连同其备注页一起移除；共享部件（布局、媒体、图表）为剩余幻灯片保留。`remove` 的索引以进入补丁时的原始 deck 顺序为准 — 与 `replace` 的键同一原始顺序；删除在追加之后执行。

## 批注

向现有幻灯片追加批注，以 0-based 幻灯片索引（`sldIdLst` 顺序）为键。每条批注复用 `generatePresentation` 的 `SlideCommentOptions` 词汇（见[批注](./18.comments.md)）。作者合并入 `commentAuthors.xml`（按名称去重，id 续编），每张幻灯片的批注合并入 `ppt/comments/commentN.xml`：
...
| `placeholders`          | `Record<string, TextRunOptions \| TextRunOptions[] \| string>` | —                            | 分隔符包裹的占位符名称 → 运行内容           |
| `findReplace`           | `Record<string, TextRunOptions \| TextRunOptions[] \| string>` | —                            | 字面查找字符串 → 运行内容（无分隔符）       |
| `coreProperties`        | `Partial<CorePropertiesOptions>`                               | —                            | 核心元数据覆盖，与现有值合并                |
| `slides`                | `{ append?, replace?, remove? }`                               | —                            | 追加幻灯片、按索引替换、或按索引删除        |
...
- 每个占位符的所有出现都会被替换，而非仅第一处 — 覆盖所有幻灯片、母版、版式和备注。
